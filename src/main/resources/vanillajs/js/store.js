...
(function (window) {
	...
	function updateItem(id, item) {
		item.id = id;
		window.fetch(URL + '/' + id, {method: 'PUT', headers: {
	...
	function deleteItem(id) {
		window.fetch(URL + '/' + id, {method: 'DELETE', headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json'
		  } })
	}
	...
	function deleteAllItems() {
		window.fetch(URL + '/drop', {method: 'DELETE', headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json'
			}
		})
...
//		}));
	...
	Store.prototype.findAll = function (callback) {
        ...
        window.fetch(URL + '/all')
        .then((response) => {return response.json();})
        .then((todos) => {callback.call(this, todos);})
...
//		var todos = JSON.parse(localStorage.getItem(this._dbName));
		...
		if (id) {
            ...
            window.fetch(URL + '/getTodo/' + id)
			.then((response) => {return response.json();})
			.then((todos) => {
				for (var key in updateData) {
					todos[key] = updateData[key];
				}
				updateItem(id, todos);
